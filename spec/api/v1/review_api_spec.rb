...
RSpec.describe V1::ReviewAPI do
  ...
  describe 'Create review' do
    subject { post "/api/v1/book/#{book.id}/review", params:  , headers: { 'authorization' => "Bearer: #{user.auth_token}" } }
  ...
  describe 'Update review' do
    ...
    subject { put "/api/v1/book/#{book.id}/review/#{review.id}", params:  , headers: { 'authorization' => "Bearer: #{user.auth_token}" } }
  ...
  describe 'Delete review' do
    ...
    subject { delete "/api/v1/book/#{book.id}/review/#{review.id}" , headers: { 'authorization' => "Bearer: #{user.auth_token}" } }
